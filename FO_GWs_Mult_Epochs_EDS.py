...
# Provide the equation of state of each epoch during reheating here in order from the earliest epoch to the latest epoch
...
# For example, if you have 3 epochs durig reheating, you have to provide 3 values in the list
EoS_list = [0.95, 0.8, 0.65] # [w_1, w_2, w_3, ..., w_n]  


# List for the energy scale of the universe in GeV at which you want each of the epochs to end except the last epoch dring reheating.
# The code will automatically add the radiation and matter domination epochs after reheating
# For example, if you have 3 epochs during reheating, you have to provide 2 values in the list

# The code is not designed for the case at which the second last epoch during to end at an energy scale < 10^2 GeV.
...
Energy_list = [10**10, 10**7] #GeV #[E_1, E_2, ...., E_{n-1}]


# Provide the temperature acheived in GeV at the end of reheating. This temperature will be automatically converted to energy scale of the universe.
# In accordance to the BBN constarint, the temperature at the end of reheating must be greater than 10^(-3) GeV. T_Rh > T_BBN = 10^(-3) GeV
T_Rh = 0.45 #GeV
...
# Provide the number of points you want in the plot of GW energy density spectrum
num_of_points = 100
...
BBN_constraint = 1.13*10**(-6)                      # Upper bound on Omega_GW from BBN observations [see Eq. (3.59)]
...
def coeff(f):
    ...
    A_k_arr[0] = 2**(alpha_arr[0]- 1/2 ) * gamma(alpha_arr[0] + 1/2)    #A_{k, 1} [see Eq. (3.34)]
    B_k_arr[0] = 0                                                      #B_{k, 1} [see Eq. (3.34)]

    for i in range(1, len(alpha_arr)):
        an_ym = alpha_arr[i] * y_arr[i-1]       # \alpha_n * y_m, where m = n-1 [see Eq. (3.18), (3.19)]
        am_ym = alpha_arr[i-1] * y_arr[i-1]     # \alpha_m * y_m, where m = n-1 [see Eq. (3.18), (3.19)]
        ...
        f_1 = mpmath.besselj(-(an_m_half), an_ym) #[see Eq. (3.14)]
        f_2 = mpmath.besselj(-(am_m_half), am_ym) #[see Eq. (3.15)]
        f_3 = mpmath.besselj(-(an_p_half), an_ym) #[see Eq. (3.16)]
        f_4 = mpmath.besselj(-(am_p_half), am_ym) #[see Eq. (3.17)]

        g_1 = mpmath.besselj(an_m_half, an_ym) #[see Eq. (3.14)]
        g_2 = mpmath.besselj(am_m_half, am_ym) #[see Eq. (3.15)]
        g_3 = mpmath.besselj(an_p_half, an_ym) #[see Eq. (3.16)]
        g_4 = mpmath.besselj(am_p_half, am_ym) #[see Eq. (3.17)]
        
        
        Deno = f_1 * g_3 + g_1 * f_3 # Denominator in Eq. (3.18) and (3.19)
        ...
        A_k_arr[i] = K * (A_k_arr[i-1] * Num_A1 + B_k_arr[i-1] * Num_B1) # [see Eq. (3.18)]
        B_k_arr[i] = K * (A_k_arr[i-1] * Num_A2 + B_k_arr[i-1] * Num_B2) # [see Eq. (3.19)]
...
const_coeff = 1/(96*(np.pi)**3) * G_R * Omega_rad_0 * (H_inf/m_P)**2 # The coefficient in Eq. (3.53)
...
end_freq = np.log10(6.3*10**7)      #Hz      #end frequency for the plot in Log10 scale
...
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Uncomment from the following lines to plot the sensitivity curves of various GW detectors
...
aLIGO_curve = LineString(np.column_stack((X1_DATA, Y1_DATA)))
Spectrum = LineString(np.column_stack((f, vec_Omega_GW_0(f))))
